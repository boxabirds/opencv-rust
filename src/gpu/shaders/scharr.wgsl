...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (x == 0u || y == 0u || x >= params.width - 1u || y >= params.height - 1u) {
        write_byte(&output, y * params.width + x, 0u);
        return;
    }
    ...
    if (params.dx > 0u) {
        for (var ky: i32 = -1; ky <= 1; ky++) {
            for (var kx: i32 = -1; kx <= 1; kx++) {
                ...
                sum += f32(read_byte(&input, idx)) * SCHARR_X[ky + 1][kx + 1];
            }
        }
    }
    ...
    if (params.dy > 0u) {
        for (var ky: i32 = -1; ky <= 1; ky++) {
            for (var kx: i32 = -1; kx <= 1; kx++) {
                ...
                let px = i32(x) + kx;
                let idx = u32(py) * params.width + u32(px);
                sum += f32(read_byte(&input, idx)) * SCHARR_Y[ky + 1][kx + 1];
            }
        }
    }

    write_byte(&output, y * params.width + x, u32(clamp(abs(sum), 0.0, 255.0)));
}
