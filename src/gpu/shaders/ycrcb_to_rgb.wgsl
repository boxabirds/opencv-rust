...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let yy = f32(read_byte(&input, idx));
    let cr = f32(read_byte(&input, idx + 1u)) - 128.0;
    let cb = f32(read_byte(&input, idx + 2u)) - 128.0;
    ...
    write_byte(&output, idx, u32(clamp(r, 0.0, 255.0)));
    write_byte(&output, idx + 1u, u32(clamp(g, 0.0, 255.0)));
    write_byte(&output, idx + 2u, u32(clamp(b, 0.0, 255.0)));
}
