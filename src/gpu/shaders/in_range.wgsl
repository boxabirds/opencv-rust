...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (params.channels == 1u) {
        let val = read_byte(&input, idx_base);
        in_range = val >= params.lower_b && val <= params.upper_b;
    } else if (params.channels == 3u) {
        let b = read_byte(&input, idx_base);
        let g = read_byte(&input, idx_base + 1u);
        let r = read_byte(&input, idx_base + 2u);
        ...
        let g = read_byte(&input, idx_base + 1u);
        let r = read_byte(&input, idx_base + 2u);
        let a = read_byte(&input, idx_base + 3u);
        in_range = b >= params.lower_b && b <= params.upper_b &&
                   g >= params.lower_g && g <= params.upper_g &&
                   r >= params.lower_r && r <= params.upper_r &&
    ...
    write_byte(&output, out_idx, select(0u, 255u, in_range));
}
