...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (params.channels >= 1u) {
        output_ch0[dst_idx] = read_byte(&input, src_idx);
    }
    if (params.channels >= 2u) {
        output_ch1[dst_idx] = read_byte(&input, src_idx + 1u);
    }
    if (params.channels >= 3u) {
        output_ch2[dst_idx] = read_byte(&input, src_idx + 2u);
    }
    if (params.channels >= 4u) {
        output_ch3[dst_idx] = read_byte(&input, src_idx + 3u);
    }
}
