...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let h = f32(read_byte(&input, idx)) * 2.0;  // Convert back to [0, 360]
    let s = f32(read_byte(&input, idx + 1u)) / 255.0;
    let v = f32(read_byte(&input, idx + 2u)) / 255.0;
    ...
    write_byte(&output, idx, u32((r + m) * 255.0));
    write_byte(&output, idx + 1u, u32((g + m) * 255.0));
    write_byte(&output, idx + 2u, u32((b + m) * 255.0));
}
