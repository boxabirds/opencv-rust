...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var c = 0u; c < params.channels; c++) {
        ...
        let value = f32(read_byte(&input, idx)) / 255.0 + 1e-6;  // Avoid log(0)
        let result = (log(value) + 6.0) * 255.0 / 6.0;  // Normalize to 0-255
        write_byte(&output, idx, u32(clamp(result, 0.0, 255.0)));
    }
}
