// Element-wise multiplication: dst = saturate(src1 * scale)
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var c: u32 = 0u; c < params.channels; c++) {
        let product = f32(input1[idx + c]) * f32(input2[idx + c]) * params.scale;
        write_byte(&output, idx + c, u32(clamp(product, 0.0, 255.0)));
    }
}
