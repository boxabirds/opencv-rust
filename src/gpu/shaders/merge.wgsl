...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (params.channels >= 1u) {
        write_byte(&output, dst_idx, input_ch0[src_idx]);
    }
    if (params.channels >= 2u) {
        write_byte(&output, dst_idx + 1u, input_ch1[src_idx]);
    }
    if (params.channels >= 3u) {
        write_byte(&output, dst_idx + 2u, input_ch2[src_idx]);
    }
    if (params.channels >= 4u) {
        write_byte(&output, dst_idx + 3u, input_ch3[src_idx]);
    }
}
