...
pub async fn median_blur_gpu_async(src: &Mat, dst: &mut Mat, ksize: i32) -> Result<()> {
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation("GPU median_blur only supports U8 depth".to_string()));
    }
    if ksize != 3 && ksize != 5 {
        return Err(Error::InvalidParameter("GPU median_blur only supports kernel sizes 3 and 5".to_string()));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), src.channels(), src.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_median_blur_impl(&temp_ctx, src, dst, ksize).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_median_blur_impl(ctx, src, dst, ksize).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn median_blur_gpu(src: &Mat, dst: &mut Mat, ksize: i32) -> Result<()> {
    pollster::block_on(median_blur_gpu_async(src, dst, ksize))
}

async fn execute_median_blur_impl(ctx: &GpuContext, src: &Mat, dst: &mut Mat, ksize: i32) -> Result<()> {
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Median Blur Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/median_blur.wgsl").into()),
    });
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("Median Blur Bind Group Layout"),
        entries: &[
    ...
    let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("Median Blur Bind Group"),
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("Median Blur Encoder"),
    });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("Median Blur Compute Pass"),
