...
pub async fn in_range_gpu_async(
    src: &Mat,
    dst: &mut Mat,
    lower_bound: Scalar,
    upper_bound: Scalar,
    ...
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation(
            "GPU inRange only supports U8 depth".to_string(),
        ));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), 1, src.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_in_range_impl(&temp_ctx, src, dst, lower_bound, upper_bound).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_in_range_impl(ctx, src, dst, lower_bound, upper_bound).await;
    }
}
...
pub fn in_range_gpu(
    ...
    pollster::block_on(in_range_gpu_async(src, dst, lower_bound, upper_bound))
}

async fn execute_in_range_impl(
    ctx: &GpuContext,
    src: &Mat,
    dst: &mut Mat,
    lower_bound: Scalar,
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/in_range.wgsl").into()),
    });
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("InRange Bind Group Layout"),
        entries: &[
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("InRange Encoder"),
    });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("InRange Compute Pass"),
