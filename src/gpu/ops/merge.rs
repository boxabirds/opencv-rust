...
pub async fn merge_gpu_async(src: &[Mat], dst: &mut Mat) -> Result<()> {
    if src.is_empty() || src.len() > 4 {
        return Err(Error::InvalidParameter(
            "Merge requires 1-4 input matrices".to_string(),
        ));
    }

    // Validate all inputs have same size and are single-channel U8
    let rows = src[0].rows();
    let cols = src[0].cols();
    for mat in src.iter() {
        if mat.depth() != MatDepth::U8 {
            return Err(Error::UnsupportedOperation(
                "GPU merge only supports U8 depth".to_string(),
            ));
        }
        if mat.channels() != 1 {
            return Err(Error::InvalidParameter(
                "All input matrices must be single-channel".to_string(),
            ));
        }
        if mat.rows() != rows || mat.cols() != cols {
            return Err(Error::InvalidParameter(
                "All input matrices must have same size".to_string(),
            ));
        }
    }

    *dst = Mat::new(rows, cols, src.len(), MatDepth::U8)?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_merge_impl(&temp_ctx, src, dst).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_merge_impl(ctx, src, dst).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn merge_gpu(src: &[Mat], dst: &mut Mat) -> Result<()> {
    pollster::block_on(merge_gpu_async(src, dst))
}

async fn execute_merge_impl(ctx: &GpuContext, src: &[Mat], dst: &mut Mat) -> Result<()> {
    let width = src[0].cols() as u32;
    let height = src[0].rows() as u32;
    let channels = src.len() as u32;
    ...
    for i in 0..4 {
        let data = if i < src.len() {
            src[i].data()
        } else {
            &vec![0u8; single_channel_size as usize]
        };

        input_buffers.push(ctx.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
    ...
    let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            ...
            wgpu::BindGroupEntry {
                binding: 3,
                resource: input_buffers[3].as_entire_binding(),
            },
            wgpu::BindGroupEntry {
                binding: 4,
