...
pub async fn ycrcb_to_rgb_gpu_async(src: &Mat, dst: &mut Mat) -> Result<()> {
    if src.channels() != 3 {
        return Err(Error::InvalidParameter("YCrCb to RGB requires 3-channel input".to_string()));
    }
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation("GPU ycrcb_to_rgb only supports U8 depth".to_string()));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), 3, MatDepth::U8)?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_ycrcb_to_rgb_impl(&temp_ctx, src, dst).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_ycrcb_to_rgb_impl(ctx, src, dst).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn ycrcb_to_rgb_gpu(src: &Mat, dst: &mut Mat) -> Result<()> {
    pollster::block_on(ycrcb_to_rgb_gpu_async(src, dst))
}

async fn execute_ycrcb_to_rgb_impl(ctx: &GpuContext, src: &Mat, dst: &mut Mat) -> Result<()> {
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("YCrCb to RGB Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/ycrcb_to_rgb.wgsl").into()),
    });
    ...
    let params = YCrCbToRgbParams { width, height, channels, _pad: 0 };
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("YCrCb to RGB Bind Group Layout"),
        entries: &[
    ...
    let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("YCrCb to RGB Bind Group"),
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("YCrCb to RGB Encoder") });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("YCrCb to RGB Compute Pass"), timestamp_writes: None });
