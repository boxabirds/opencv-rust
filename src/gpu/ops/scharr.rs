...
pub async fn scharr_gpu_async(src: &Mat, dst: &mut Mat, dx: i32, dy: i32) -> Result<()> {
    if src.channels() != 1 {
        return Err(Error::InvalidParameter("Scharr only works on single-channel images".to_string()));
    }
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation("GPU scharr only supports U8 depth".to_string()));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), 1, MatDepth::U8)?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_scharr_impl(&temp_ctx, src, dst, dx, dy).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_scharr_impl(ctx, src, dst, dx, dy).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn scharr_gpu(src: &Mat, dst: &mut Mat, dx: i32, dy: i32) -> Result<()> {
    pollster::block_on(scharr_gpu_async(src, dst, dx, dy))
}

async fn execute_scharr_impl(ctx: &GpuContext, src: &Mat, dst: &mut Mat, dx: i32, dy: i32) -> Result<()> {
    let width = u32::try_from(src.cols()).unwrap_or(u32::MAX);
    let height = u32::try_from(src.rows()).unwrap_or(u32::MAX);

    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Scharr Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/scharr.wgsl").into()),
    });
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("Scharr Bind Group Layout"),
        entries: &[
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("Scharr Encoder"),
    });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("Scharr Compute Pass"),
