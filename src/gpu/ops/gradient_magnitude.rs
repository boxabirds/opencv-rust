...
struct GradientMagnitudeParams {
    width: u32,
    height: u32,
    _pad0: u32,
    _pad1: u32,
...
pub async fn gradient_magnitude_gpu_async(src: &Mat, dst: &mut Mat) -> Result<()> {
    if src.channels() != 1 {
        return Err(Error::InvalidParameter(
            "Gradient magnitude requires single-channel input".to_string(),
        ));
    }
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation(
            "GPU gradient_magnitude only supports U8 depth".to_string(),
        ));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), 1, MatDepth::U8)?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_gradient_magnitude_impl(&temp_ctx, src, dst).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_gradient_magnitude_impl(ctx, src, dst).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn gradient_magnitude_gpu(src: &Mat, dst: &mut Mat) -> Result<()> {
    pollster::block_on(gradient_magnitude_gpu_async(src, dst))
}
...
async fn execute_gradient_magnitude_impl(ctx: &GpuContext, src: &Mat, dst: &mut Mat) -> Result<()> {
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/gradient_magnitude.wgsl").into()),
    });
    ...
    let params = GradientMagnitudeParams {
        width,
        height,
        _pad0: 0,
        _pad1: 0,
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("GradientMagnitude Bind Group Layout"),
        entries: &[
    ...
    let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("GradientMagnitude Bind Group"),
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("GradientMagnitude Encoder"),
    });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("GradientMagnitude Compute Pass"),
