...
pub async fn normalize_gpu_async(
    src: &Mat,
    dst: &mut Mat,
    alpha: f64,
    beta: f64,
    ...
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation(
            "GPU normalize only supports U8 depth".to_string(),
        ));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), src.channels(), src.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_normalize_impl(&temp_ctx, src, dst, alpha, beta).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_normalize_impl(ctx, src, dst, alpha, beta).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn normalize_gpu(src: &Mat, dst: &mut Mat, alpha: f64, beta: f64) -> Result<()> {
    pollster::block_on(normalize_gpu_async(src, dst, alpha, beta))
}

async fn execute_normalize_impl(
    ctx: &GpuContext,
    src: &Mat,
    dst: &mut Mat,
    alpha: f64,
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/normalize.wgsl").into()),
    });
