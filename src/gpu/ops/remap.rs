...
pub async fn remap_gpu_async(
    ...
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation(
            "GPU remap only supports U8 depth".to_string(),
        ));
    }

    if map_x.depth() != MatDepth::F32 || map_y.depth() != MatDepth::F32 {
        return Err(Error::InvalidParameter(
            "Map matrices must be F32 depth".to_string(),
        ));
    }

    if map_x.channels() != 1 || map_y.channels() != 1 {
        return Err(Error::InvalidParameter(
            "Map matrices must be single-channel".to_string(),
        ));
    }

    if map_x.rows() != src.rows() || map_x.cols() != src.cols() ||
       map_y.rows() != src.rows() || map_y.cols() != src.cols() {
        return Err(Error::InvalidParameter(
            "Map matrices must have same size as source".to_string(),
        ));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), src.channels(), src.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_remap_impl(&temp_ctx, src, dst, map_x, map_y).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_remap_impl(ctx, src, dst, map_x, map_y).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn remap_gpu(src: &Mat, dst: &mut Mat, map_x: &Mat, map_y: &Mat) -> Result<()> {
    pollster::block_on(remap_gpu_async(src, dst, map_x, map_y))
}

async fn execute_remap_impl(
    ctx: &GpuContext,
    src: &Mat,
    dst: &mut Mat,
    map_x: &Mat,
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/remap.wgsl").into()),
    });
    ...
    let map_x_buffer = ctx.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("Map X Buffer"),
        contents: map_x.data(),
        usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
    });

    let map_y_buffer = ctx.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("Map Y Buffer"),
        contents: map_y.data(),
        usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
    });
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("Remap Bind Group Layout"),
        entries: &[
    ...
    let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            ...
            wgpu::BindGroupEntry {
                binding: 1,
                resource: map_x_buffer.as_entire_binding(),
            },
            wgpu::BindGroupEntry {
                binding: 2,
                resource: map_y_buffer.as_entire_binding(),
            },
            wgpu::BindGroupEntry {
                binding: 3,
    ...
    let pipeline_layout = ctx.device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("Remap Pipeline Layout"),
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("Remap Encoder"),
    });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("Remap Compute Pass"),
