...
pub async fn pow_gpu_async(src: &Mat, dst: &mut Mat, power: f64) -> Result<()> {
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation(
            "GPU pow only supports U8 depth".to_string(),
        ));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), src.channels(), src.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_pow_impl(&temp_ctx, src, dst, power).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_pow_impl(ctx, src, dst, power).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn pow_gpu(src: &Mat, dst: &mut Mat, power: f64) -> Result<()> {
    pollster::block_on(pow_gpu_async(src, dst, power))
}

async fn execute_pow_impl(ctx: &GpuContext, src: &Mat, dst: &mut Mat, power: f64) -> Result<()> {
    let width = src.cols() as u32;
    let height = src.rows() as u32;
    let channels = src.channels() as u32;

    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Pow Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/pow.wgsl").into()),
    });
