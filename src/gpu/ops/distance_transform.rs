...
struct DistanceTransformParams {
    width: u32,
    height: u32,
    max_dist: f32,
    _pad: u32,
...
pub async fn distance_transform_gpu_async(src: &Mat, dst: &mut Mat) -> Result<()> {
    if src.channels() != 1 {
        return Err(Error::InvalidParameter(
            "Distance transform requires single-channel input".to_string(),
        ));
    }
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation(
            "GPU distance_transform only supports U8 depth".to_string(),
        ));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), 1, MatDepth::F32)?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_distance_transform_impl(&temp_ctx, src, dst).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_distance_transform_impl(ctx, src, dst).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn distance_transform_gpu(src: &Mat, dst: &mut Mat) -> Result<()> {
    pollster::block_on(distance_transform_gpu_async(src, dst))
}

async fn execute_distance_transform_impl(ctx: &GpuContext, src: &Mat, dst: &mut Mat) -> Result<()> {
    let width = src.cols() as u32;
    let height = src.rows() as u32;
    let max_dist = ((width * width + height * height) as f32).sqrt();

    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("DistanceTransform Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/distance_transform.wgsl").into()),
    });
    ...
    let params = DistanceTransformParams {
        width,
        height,
        max_dist,
        _pad: 0,
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("DistanceTransform Bind Group Layout"),
        entries: &[
    ...
    let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("DistanceTransform Bind Group"),
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("DistanceTransform Encoder"),
    });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("DistanceTransform Compute Pass"),
