...
pub async fn bitwise_or_gpu_async(src1: &Mat, src2: &Mat, dst: &mut Mat) -> Result<()> {
    if src1.depth() != MatDepth::U8 || src2.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation("GPU bitwise_or only supports U8 depth".to_string()));
    }
    ...
    *dst = Mat::new(src1.rows(), src1.cols(), src1.channels(), src1.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_bitwise_or_impl(&temp_ctx, src1, src2, dst).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_bitwise_or_impl(ctx, src1, src2, dst).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn bitwise_or_gpu(src1: &Mat, src2: &Mat, dst: &mut Mat) -> Result<()> {
    pollster::block_on(bitwise_or_gpu_async(src1, src2, dst))
}

async fn execute_bitwise_or_impl(ctx: &GpuContext, src1: &Mat, src2: &Mat, dst: &mut Mat) -> Result<()> {
    let width = src1.cols() as u32;
    let height = src1.rows() as u32;
    let channels = src1.channels() as u32;

    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("BitwiseOr Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/bitwise_or.wgsl").into()),
    });
    ...
    let params = BitwiseOrParams { width, height, channels, _pad: 0 };
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("BitwiseOr Bind Group Layout"),
        entries: &[
    ...
    let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("BitwiseOr Bind Group"),
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("BitwiseOr Encoder") });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("BitwiseOr Compute Pass"), timestamp_writes: None });
