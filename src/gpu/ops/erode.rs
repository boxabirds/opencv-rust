...
pub async fn erode_gpu_async(src: &Mat, dst: &mut Mat, ksize: i32) -> Result<()> {
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation("GPU erode only supports U8 depth".to_string()));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), src.channels(), src.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_erode_impl(&temp_ctx, src, dst, ksize).await;
    }

    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get().ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_erode_impl(ctx, src, dst, ksize).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn erode_gpu(src: &Mat, dst: &mut Mat, ksize: i32) -> Result<()> {
    pollster::block_on(erode_gpu_async(src, dst, ksize))
}

async fn execute_erode_impl(ctx: &GpuContext, src: &Mat, dst: &mut Mat, ksize: i32) -> Result<()> {
    ...
    let params = ErodeParams { width, height, channels, kernel_size: ksize as u32 };
