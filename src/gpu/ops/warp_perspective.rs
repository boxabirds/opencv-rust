...
pub async fn warp_perspective_gpu_async(
    src: &Mat,
    dst: &mut Mat,
    ...
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation(
            "GPU warpPerspective only supports U8 depth".to_string(),
        ));
    }

    let (dst_cols, dst_rows) = dsize;
    *dst = Mat::new(dst_rows, dst_cols, src.channels(), src.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_warp_perspective_impl(&temp_ctx, src, dst, perspective_matrix).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_warp_perspective_impl(ctx, src, dst, perspective_matrix).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn warp_perspective_gpu(
    src: &Mat,
    dst: &mut Mat,
    perspective_matrix: &[f32; 9],
    dsize: (usize, usize),
) -> Result<()> {
    pollster::block_on(warp_perspective_gpu_async(src, dst, perspective_matrix, dsize))
}

async fn execute_warp_perspective_impl(
    ctx: &GpuContext,
    src: &Mat,
    dst: &mut Mat,
    perspective_matrix: &[f32; 9],
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/warp_perspective.wgsl").into()),
    });
    ...
    let params = WarpPerspectiveParams {
        ...
        m01: perspective_matrix[1],
        m02: perspective_matrix[2],
        m10: perspective_matrix[3],
        m11: perspective_matrix[4],
        m12: perspective_matrix[5],
        m20: perspective_matrix[6],
        m21: perspective_matrix[7],
        m22: perspective_matrix[8],
        _pad2: 0.0,
        _pad3: 0.0,
        _pad4: 0.0,
    };
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("Warp Perspective Bind Group Layout"),
        entries: &[
    ...
    let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("Warp Perspective Bind Group"),
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("Warp Perspective Encoder"),
    });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("Warp Perspective Compute Pass"),
