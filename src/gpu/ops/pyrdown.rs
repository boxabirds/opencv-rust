...
pub async fn pyrdown_gpu_async(src: &Mat, dst: &mut Mat) -> Result<()> {
    ...
    *dst = Mat::new(dst_height, dst_width, src.channels(), src.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_pyrdown_impl(&temp_ctx, src, dst).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_pyrdown_impl(ctx, src, dst).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn pyrdown_gpu(src: &Mat, dst: &mut Mat) -> Result<()> {
    pollster::block_on(pyrdown_gpu_async(src, dst))
}

async fn execute_pyrdown_impl(ctx: &GpuContext, src: &Mat, dst: &mut Mat) -> Result<()> {
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("PyrDown Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/pyrdown.wgsl").into()),
    });
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("PyrDown Bind Group Layout"),
        entries: &[
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("PyrDown Encoder"),
    });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("PyrDown Compute Pass"),
