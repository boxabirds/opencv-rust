...
pub async fn sqrt_gpu_async(src: &Mat, dst: &mut Mat) -> Result<()> {
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation(
            "GPU sqrt only supports U8 depth".to_string(),
        ));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), src.channels(), src.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_sqrt_impl(&temp_ctx, src, dst).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_sqrt_impl(ctx, src, dst).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn sqrt_gpu(src: &Mat, dst: &mut Mat) -> Result<()> {
    pollster::block_on(sqrt_gpu_async(src, dst))
}

async fn execute_sqrt_impl(ctx: &GpuContext, src: &Mat, dst: &mut Mat) -> Result<()> {
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Sqrt Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/sqrt.wgsl").into()),
    });
