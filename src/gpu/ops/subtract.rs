...
pub async fn subtract_gpu_async(src1: &Mat, src2: &Mat, dst: &mut Mat) -> Result<()> {
    if src1.depth() != MatDepth::U8 || src2.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation("GPU subtract only supports U8 depth".to_string()));
    }
    ...
    *dst = Mat::new(src1.rows(), src1.cols(), src1.channels(), src1.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_subtract_impl(&temp_ctx, src1, src2, dst).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_subtract_impl(ctx, src1, src2, dst).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn subtract_gpu(src1: &Mat, src2: &Mat, dst: &mut Mat) -> Result<()> {
    pollster::block_on(subtract_gpu_async(src1, src2, dst))
}

async fn execute_subtract_impl(ctx: &GpuContext, src1: &Mat, src2: &Mat, dst: &mut Mat) -> Result<()> {
    let width = src1.cols() as u32;
    let height = src1.rows() as u32;
    let channels = src1.channels() as u32;

    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Subtract Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/subtract.wgsl").into()),
    });
    ...
    let params = SubtractParams { width, height, channels, _pad: 0 };
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("Subtract Bind Group Layout"),
        entries: &[
    ...
    let pipeline_layout = ctx.device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
        label: Some("Subtract Pipeline Layout"),
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("Subtract Encoder") });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("Subtract Compute Pass"), timestamp_writes: None });
