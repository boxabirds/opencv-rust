...
pub async fn convert_scale_gpu_async(
    src: &Mat,
    dst: &mut Mat,
    alpha: f64,
    beta: f64,
    ...
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation(
            "GPU convert_scale only supports U8 depth".to_string(),
        ));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), src.channels(), src.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_convert_scale_impl(&temp_ctx, src, dst, alpha, beta).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_convert_scale_impl(ctx, src, dst, alpha, beta).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn convert_scale_gpu(src: &Mat, dst: &mut Mat, alpha: f64, beta: f64) -> Result<()> {
    pollster::block_on(convert_scale_gpu_async(src, dst, alpha, beta))
}

async fn execute_convert_scale_impl(
    ctx: &GpuContext,
    src: &Mat,
    dst: &mut Mat,
    alpha: f64,
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("ConvertScale Shader"),
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/convert_scale.wgsl").into()),
    });
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("ConvertScale Bind Group Layout"),
        entries: &[
    ...
    let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("ConvertScale Bind Group"),
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("ConvertScale Encoder"),
    });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("ConvertScale Compute Pass"),
