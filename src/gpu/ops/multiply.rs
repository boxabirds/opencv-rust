...
pub async fn multiply_gpu_async(src1: &Mat, src2: &Mat, dst: &mut Mat, scale: f64) -> Result<()> {
    if src1.depth() != MatDepth::U8 || src2.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation("GPU multiply only supports U8 depth".to_string()));
    }
    ...
    *dst = Mat::new(src1.rows(), src1.cols(), src1.channels(), src1.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_multiply_impl(&temp_ctx, src1, src2, dst, scale).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_multiply_impl(ctx, src1, src2, dst, scale).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn multiply_gpu(src1: &Mat, src2: &Mat, dst: &mut Mat, scale: f64) -> Result<()> {
    pollster::block_on(multiply_gpu_async(src1, src2, dst, scale))
}

async fn execute_multiply_impl(ctx: &GpuContext, src1: &Mat, src2: &Mat, dst: &mut Mat, scale: f64) -> Result<()> {
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/multiply.wgsl").into()),
    });
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("Multiply Encoder") });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("Multiply Compute Pass"), timestamp_writes: None });
