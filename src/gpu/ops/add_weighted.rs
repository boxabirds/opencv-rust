...
pub async fn add_weighted_gpu_async(
    src1: &Mat,
    alpha: f64,
    src2: &Mat,
    beta: f64,
    ...
    if src1.depth() != MatDepth::U8 || src2.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation(
            "GPU add_weighted only supports U8 depth".to_string(),
        ));
    }
    if src1.rows() != src2.rows() || src1.cols() != src2.cols() || src1.channels() != src2.channels() {
        return Err(Error::InvalidParameter(
            "Input images must have same dimensions and channels".to_string(),
        ));
    }
    ...
    *dst = Mat::new(src1.rows(), src1.cols(), src1.channels(), src1.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_add_weighted_impl(&temp_ctx, src1, alpha, src2, beta, gamma, dst).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_add_weighted_impl(ctx, src1, alpha, src2, beta, gamma, dst).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn add_weighted_gpu(
    src1: &Mat,
    alpha: f64,
    src2: &Mat,
    beta: f64,
    ...
    pollster::block_on(add_weighted_gpu_async(src1, alpha, src2, beta, gamma, dst))
}

async fn execute_add_weighted_impl(
    ctx: &GpuContext,
    src1: &Mat,
    alpha: f64,
    src2: &Mat,
    ...
    let shader = ctx.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/add_weighted.wgsl").into()),
    });
    ...
    let bind_group_layout = ctx.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        label: Some("AddWeighted Bind Group Layout"),
        entries: &[
    ...
    let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("AddWeighted Bind Group"),
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("AddWeighted Encoder"),
    });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("AddWeighted Compute Pass"),
