...
pub async fn flip_gpu_async(src: &Mat, dst: &mut Mat, flip_code: i32) -> Result<()> {
    if src.depth() != MatDepth::U8 {
        return Err(Error::UnsupportedOperation("GPU flip only supports U8 depth".to_string()));
    }
    ...
    *dst = Mat::new(src.rows(), src.cols(), src.channels(), src.depth())?;
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        return execute_flip_impl(&temp_ctx, src, dst, flip_code).await;
    }
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        let ctx = GpuContext::get()
            .ok_or_else(|| Error::GpuNotAvailable("GPU context not initialized".to_string()))?;
        return execute_flip_impl(ctx, src, dst, flip_code).await;
    }
}

#[cfg(not(target_arch = "wasm32"))]
pub fn flip_gpu(src: &Mat, dst: &mut Mat, flip_code: i32) -> Result<()> {
    pollster::block_on(flip_gpu_async(src, dst, flip_code))
}
...
async fn execute_flip_impl(ctx: &GpuContext, src: &Mat, dst: &mut Mat, flip_code: i32) -> Result<()> {
    ...
    let params = FlipParams { width, height, channels, flip_code };
    ...
    let (bind_group_layout, compute_pipeline) = {
        let cached = PipelineCache::get_flip_pipeline()
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("Flip Encoder") });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("Flip Compute Pass"), timestamp_writes: None });
